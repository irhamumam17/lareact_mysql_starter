...
function SortHeader({ label, columnKey }: { label: string; columnKey: string }) {
    ...
    return (
        <Link
            href={regencies.index.url({ mergeQuery: { sort: columnKey, direction: nextDirection, page: 1, per_page: currentPerPage || undefined } })}
...
export const columns: ColumnDef<RegencyModel>[] = [
    {
        accessorKey: "name",
    ...
    },
    {
        ...
        header: "Province",
        cell: ({ row }) => {
            return <span>{row.original.province?.name ?? '-'}</span>;
        },
    },
    {
