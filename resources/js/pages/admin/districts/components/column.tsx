...
function SortHeader({ label, columnKey }: { label: string; columnKey: string }) {
    ...
    return (
        <Link
            href={districts.index.url({ mergeQuery: { sort: columnKey, direction: nextDirection, page: 1, per_page: currentPerPage || undefined } })}
...
export const columns: ColumnDef<DistrictModel>[] = [
    {
        accessorKey: "name",
    ...
    },
    {
        ...
        header: "Regency",
        cell: ({ row }) => {
            return <span>{row.original.regency?.name ?? '-'}</span>;
        },
    },
    {
